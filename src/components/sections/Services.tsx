...
export function Services() {
  ...
  return (
    <section id="expertise" className="py-28 lg:py-36">
      <Container>
        <Reveal>
          <SectionHeading
            eyebrow="Expertise"
            title={
              <>
                Where <span className="gradient-text">eight years</span> compounds
              </>
            }
            description="Three core disciplines — web platforms, AI engineering, and mobile — plus the delivery systems that keep them production-ready."
          />
        </Reveal>
          ...
          {serviceStats.map((stat) => (
            <StaggerItem key={stat.label}>
              <div className="glass-card p-5 text-center transition hover:-translate-y-1">
                <p className="font-[family-name:var(--font-mono)] text-[10px] uppercase tracking-[0.16em] text-muted-foreground">
                  {stat.label}
                </p>
...
function ServiceGroup({
  ...
  return (
    <div className="mt-14">
      <h3 className="mb-6 font-[family-name:var(--font-mono)] text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground">
        {title}
      </h3>
          ...
          <Reveal key={service.id} delay={Math.min(index * 0.06, 0.24)}>
            <TiltCard className="group h-full">
              <article className="glass-card-hover flex h-full flex-col p-6">
                <div className="mb-4 h-1 w-10 bg-primary transition-all duration-300 group-hover:w-20" />
                <h4 className="font-[family-name:var(--font-display)] text-xl font-bold">
                  {service.title}
                </h4>
